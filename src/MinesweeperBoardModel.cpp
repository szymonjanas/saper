...
MinesweeperBoard::MinesweeperBoard(GameMode mode)
    : width(9), heigh(7)
{
    stateOfGame = GameState::RUNNING;
...
char MinesweeperBoard::getFieldInfo(int x, int y) const
{
    ...
    if (isRevealed(x,y) and numberOfMines == 0) return ' ';
    if (isRevealed(x,y) and numberOfMines > 0 and numberOfMines < 9) return (char)(48 + numberOfMines);
    return 'E';
}
